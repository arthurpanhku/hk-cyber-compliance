/**
 * 条文出处登记。所有链接与发布日期于 2026-09-08 经官方网站核验。
 * status: current = 现行有效；ref = 背景/参考文件
 */
HKCC.addSources({
  'sfc-hacking': {
    regulator: 'SFC',
    titleEn: 'Guidelines for Reducing and Mitigating Hacking Risks Associated with Internet Trading',
    titleZh: '减低及缓减与互联网交易相关的黑客入侵风险指引',
    issued: '2017-10-27',
    legalStatus: '根据《证券及期货条例》第 399 条发出的指引；不具法律效力，但违反其精神可能影响持牌人的适当人选资格',
    url: 'https://www.sfc.hk/-/media/EN/assets/components/codes/files-current/web/guidelines/guidelines-for-reducing-and-mitigating-hacking-risks-associated-with-internet-trading/guidelines-for-reducing-and-mitigating-hacking-risks-associated-with-internet-trading.pdf',
    status: 'current'
  },
  'sfc-cir-phishing-2026': {
    regulator: 'SFC',
    titleEn: 'Circular — Implementing (i) robust authentication methods to reduce and mitigate hacking risks from phishing attacks and (ii) adequate monitoring and surveillance measures to identify suspicious activities',
    titleZh: '通函——实施 (i) 稳健的认证方法以减低及缓减钓鱼攻击的黑客风险，及 (ii) 充分的监控措施以识别可疑活动',
    ref: '26EC35',
    issued: '2026-07-09',
    legalStatus: '证监会通函，载列预期操守标准',
    url: 'https://apps.sfc.hk/edistributionWeb/gateway/EN/circular/intermediaries/supervision/doc?refNo=26EC35',
    status: 'current'
  },
  'sfc-cir-ai-2026': {
    regulator: 'SFC',
    titleEn: 'Circular — Enhanced cybersecurity measures to address evolving risks arising from artificial intelligence-enabled cyberattacks',
    titleZh: '通函——加强网络安全措施以应对人工智能驱动网络攻击带来的新兴风险',
    ref: '26EC32',
    issued: '2026-06-02',
    legalStatus: '证监会通函，载列预期操守标准',
    url: 'https://apps.sfc.hk/edistributionWeb/gateway/EN/circular/intermediaries/supervision/doc?refNo=26EC32',
    status: 'current'
  },
  'sfc-cir-mapping-2026': {
    regulator: 'SFC / HKMA / IA',
    titleEn: 'Joint Circular on the Cross-Sectoral Cyber Mapping Exercise',
    titleZh: '跨界别网络测绘工作联合通函',
    ref: '26EC45',
    issued: '2026-07-29',
    legalStatus: '跨监管机构联合通函',
    url: 'https://apps.sfc.hk/edistributionWeb/gateway/EN/circular/intermediaries/supervision/doc?refNo=26EC45',
    status: 'ref'
  },
  'sfc-code-of-conduct': {
    regulator: 'SFC',
    titleEn: 'Code of Conduct for Persons Licensed by or Registered with the SFC — paragraph 18 and Schedule 7',
    titleZh: '《证监会持牌人或注册人操守准则》第 18 段及附表 7',
    issued: '—',
    legalStatus: '操守准则；违反可影响适当人选资格并招致纪律行动',
    url: 'https://www.sfc.hk/en/Rules-and-standards/Codes-and-guidelines/Codes',
    status: 'current'
  },
  'hkma-tm-g-1': {
    regulator: 'HKMA',
    titleEn: 'SPM TM-G-1 General Principles for Technology Risk Management',
    titleZh: '《监管政策手册》TM-G-1 科技风险管理一般原则',
    issued: '2003-06-24',
    legalStatus: '金管专员以指导注释形式发出的非法定指引',
    url: 'https://brdr.hkma.gov.hk/eng/doc-ldg/spm/current/TM-G-1',
    status: 'current'
  },
  'hkma-tm-e-1': {
    regulator: 'HKMA',
    titleEn: 'SPM TM-E-1 Risk Management of E-banking (V.4)',
    titleZh: '《监管政策手册》TM-E-1 电子银行风险管理（第 4 版）',
    issued: '2024-10-25',
    legalStatus: '根据《银行业条例》第 7(3) 条发出的法定指引',
    url: 'https://brdr.hkma.gov.hk/eng/doc-ldg/spm/current/TM-E-1',
    status: 'current'
  },
  'hkma-tm-c-1': {
    regulator: 'HKMA',
    titleEn: 'SPM TM-C-1 Supervisory Approach on Cyber Risk Management',
    titleZh: '《监管政策手册》TM-C-1 网络风险管理的监管方针',
    issued: '2024-11-29',
    legalStatus: '根据《银行业条例》第 7(3) 条发出的法定指引；取代 2015-09-15《网络安全风险管理》通函',
    url: 'https://brdr.hkma.gov.hk/eng/doc-ldg/docId/20241202-2-EN',
    status: 'current'
  },
  'hkma-or-2': {
    regulator: 'HKMA',
    titleEn: 'SPM OR-2 Operational Resilience',
    titleZh: '《监管政策手册》OR-2 营运韧性',
    issued: '2022-05-31',
    legalStatus: '金管专员以指导注释形式发出的非法定指引',
    url: 'https://brdr.hkma.gov.hk/eng/doc-ldg/spm/current/OR-2',
    status: 'current'
  },
  'hkma-sa-2': {
    regulator: 'HKMA',
    titleEn: 'SPM SA-2 Outsourcing',
    titleZh: '《监管政策手册》SA-2 外判',
    issued: '2001-12-28',
    legalStatus: '金管专员以指导注释形式发出的非法定指引',
    url: 'https://brdr.hkma.gov.hk/eng/doc-ldg/spm/current/SA-2',
    status: 'current'
  },
  'hkma-cfi-2': {
    regulator: 'HKMA',
    titleEn: 'Circular — Cybersecurity Fortification Initiative 2.0 (C-RAF 2.0)',
    titleZh: '通函——网络安全强化计划 2.0（网络防卫评估架构 C-RAF 2.0）',
    issued: '2020-11-03',
    legalStatus: '金管局通函；C-RAF 评估为认可机构的持续监管要求',
    url: 'https://brdr.hkma.gov.hk/eng/doc-ldg/docId/20201103-1-EN',
    status: 'current'
  },
  'hkma-cir-ai-2026': {
    regulator: 'HKMA',
    titleEn: 'Circular — Strengthening Cyber Resilience amid Artificial Intelligence-Empowered Cyber Threats',
    titleZh: '通函——在人工智能驱动的网络威胁下强化网络韧性',
    issued: '2026-06-02',
    legalStatus: '金管局通函',
    url: 'https://brdr.hkma.gov.hk/eng/doc-ldg/docId/20260529-8-EN',
    status: 'current'
  },
  'hkma-cop-ci': {
    regulator: 'HKMA',
    titleEn: 'Code of Practice Pursuant to the Protection of Critical Infrastructures (Computer Systems) Ordinance (for Authorized Institutions designated by the MA as Critical Infrastructure Operators)',
    titleZh: '《保护关键基础设施（电脑系统）条例》实务守则（适用于金管专员指定为关键基础设施营运者的认可机构）',
    issued: '2026-06-02',
    legalStatus: '金管专员根据条例第 8(1)(b) 条发出的界别实务守则',
    url: 'https://brdr.hkma.gov.hk/eng/doc-ldg/docId/20260527-25-EN',
    status: 'current'
  },
  'occics-cop': {
    regulator: '关键基础设施（电脑系统安全）专员办公室',
    titleEn: 'Code of Practice Pursuant to the Protection of Critical Infrastructures (Computer Systems) Ordinance (Generic), Version 1.0',
    titleZh: '《保护关键基础设施（电脑系统）条例》实务守则（通用版）第 1.0 版',
    issued: '2026-01-01',
    legalStatus: '专员根据条例第 8 条发出的实务守则；本身非附属法例，但专员可就不遵从发出书面指示，不遵从指示即属犯罪',
    url: 'https://www.occics.gov.hk/filemanager/en/content_19/CoP_en_v1.0.pdf',
    status: 'current'
  },
  'occics-index': {
    regulator: '关键基础设施（电脑系统安全）专员办公室',
    titleEn: 'Codes of Practice index (generic and sectoral)',
    titleZh: '实务守则索引（通用版及界别版）',
    issued: '2026-06-22',
    legalStatus: '官方索引页',
    url: 'https://www.occics.gov.hk/en/industry/code-of-practice/index.html',
    status: 'ref'
  },
  'pdpo': {
    regulator: 'PCPD',
    titleEn: 'Personal Data (Privacy) Ordinance (Cap. 486) — Six Data Protection Principles',
    titleZh: '《个人资料（私隐）条例》（第 486 章）——六项保障资料原则',
    issued: '—',
    legalStatus: '法例；DPP4 资料保安原则对所有资料使用者具约束力',
    url: 'https://www.pcpd.org.hk/english/data_privacy_law/6_data_protection_principles/principles.html',
    status: 'current'
  },
  'sfc-vatp-guidelines': {
    regulator: 'SFC',
    titleEn: 'Guidelines for Virtual Asset Trading Platform Operators',
    titleZh: '《虚拟资产交易平台营运者指引》',
    issued: '—',
    legalStatus: '证监会指引',
    url: 'https://www.sfc.hk/-/media/EN/assets/components/codes/files-current/web/guidelines/Guidelines-for-Virtual-Asset-Trading-Platform-Operators/Guidelines-for-Virtual-Asset-Trading-Platform-Operators.pdf',
    status: 'ref'
  }
});
